...
from .models import Event,Booking,CheckOut
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if check_total_platinum_seats<total_paltinum_seats and check_total_gold_seats<total_gold_seats and check_total_silver_seats<total_silver_seats:
                ...
                messages.info(request, message)
                return HttpResponseRedirect(reverse('home:checkout',args=(visitor_name,phone_number,event_name,platinum_seats,gold_seats,silver_seats, total_platinum_price, total_gold_price,total_silver_price,net_price,booked_date)))
...
def checkout(request,visitor_name,phone_number,event_name,platinum_seats,gold_seats,silver_seats, total_platinum_price, total_gold_price,total_silver_price,net_price,booked_date):
...
def checkout1(request):
    if request.method=="POST":
        ...
        inc = CheckOut(name=name , phone=phone , email=email , address=address , city=city , state=state , zip_code=zip_code , net_price=net_price)
        inc.save()
        id = inc.booking_id
        param_dict = {

            'MID': 'WorldP64425807474247',
            'ORDER_ID': str(inc.booking_id),
            'TXN_AMOUNT': str(net_price),
            'CUST_ID': email,
